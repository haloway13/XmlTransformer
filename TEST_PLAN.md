...
| TC-08: Directory Navigation | [ ] | [x] | [x] | Linux & macOS Passed | subfolder/ shown and navigable; .hidden/ correctly excluded from quick panel; Parent Directory returns correctly. macOS: subfolder/ listed and navigable (shows its test.xsl); .hidden/ (containing an .xsl) excluded; [Parent Directory] returns to test_xslt/. |
...
| TC-10: Error Panel Display | [ ] | [x] | [x] | Linux & macOS Passed | Found & fixed 1 bug: invalid XML was only validated deep inside run_transformation() (after XSL + param prompts), not immediately on Ctrl+B; moved validation earlier. Both invalid-XML and invalid-XSL cases now show clean descriptive alerts with no traceback.. macOS: Found & fixed 1 bug affecting all platforms (Windows/Linux should re-verify the Saxon-error case): XmlTransformer_exec.check_output() looked for the output file 500ms after launching Java, so a test-output.* left from a previous run was opened while Saxon was still running (~1.3s here) and error handling was skipped entirely; a failed transform looked like success showing stale output, and the xml_transformer_errors panel never appeared. It also only treated the run as finished if the last panel line was [Finished ...], but Sublime appends [cmd:]/[dir:]/[path:] lines on non-zero exit, and its jump-to-line regex didn't match Saxon 12's 'on line N column M of file' format. Now waits for [Finished ...], uses the exit code, opens output only on exit 0, and on failure shows the panel and jumps to the XSL error position. Verified: invalid XML alerts immediately on Cmd+B; non-well-formed XSL alerts before Java runs; well-formed invalid XSLT (XPST0003) shows xml_transformer_errors panel, opens the XSL at line 6 col 45, no stale output opened; normal transform still opens output. |
