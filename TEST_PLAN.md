...
| :--- | :---: | :---: | :---: | :---: | :--- |
| Automated Setup Script | [x] | [x] | [x] | All Platforms Passed | Windows: setup_XmlTransformer_windows.bat verified with !ERRORLEVEL! delayed expansion fix; downloads Saxon-HE 12.10, xmlresolver 6.0.23 (+ data jar) to C:\Program Files\Saxon. Linux: Java present (OpenJDK 25), JARs downloaded to /usr/local/lib/saxon at 644 perms. macOS: temurin@17 cask fallback and permissions verified; JARs downloaded to ~/Library/Saxon. |
| Java 17+ Detection | [x] | [x] | [x] | All Platforms Passed | Windows: Detected Java 17+ runtime. Linux: Detected OpenJDK 25. macOS: Queries java_home first, falls back to brew paths; Temurin 17 selected. |
| Saxon 12.10 Detection | [x] | [x] | [x] | All Platforms Passed | Windows: Detected in C:\Program Files\Saxon. Linux: Detected in /usr/local/lib/saxon. macOS: Detected in ~/Library/Saxon alongside 12.9. |
| xmlresolver 6.0.23 Detection | [x] | [x] | [x] | All Platforms Passed | Windows: Detected in C:\Program Files\Saxon. Linux: Detected in /usr/local/lib/saxon. macOS: Detected in ~/Library/Saxon alongside 6.0.6. |
| TC-01: Plugin Load | [x] | [x] | [x] | All Platforms Passed | Windows: Clean load on ST4 build 4200 (windows x64); Java and JAR paths logged, no tracebacks. Linux: Clean initialization on ST4 build 4200 (linux x64). macOS: Clean load on ST4 build 4200 (osx x64). |
| TC-02: Version Sorting | [x] | [x] | [x] | All Platforms Passed | Windows: Prioritizes 12.10/6.0.23 over 12.9/6.0.6; fallback verified. Linux & macOS: Prioritizes 12.10/6.0.23, correctly falls back to legacy JARs when newer JARs removed and re-detects once restored. |
| TC-03: Missing Dependency Alert | [x] | [x] | [x] | All Platforms Passed | Windows: Renaming Saxon directory triggered pop-up dialog identifying C:\Program Files\Saxon and setup_XmlTransformer_windows.bat; restoration verified. Linux & macOS: Wired platform-specific install hints; missing Java and missing JARs dialogs verified clean. |
| TC-04: Param File Transformation| [x] | [x] | [x] | All Platforms Passed | Windows: HTML output generated with parameter values from params.xml. Linux & macOS: Verified transformation with parameter bindings. |
| TC-05: Manual Param Entry | [x] | [x] | [x] | All Platforms Passed | Windows: Manual entry prompted for each param, saved to custom_params.xml with proper indentation, rendered in HTML. Linux & macOS: Implemented missing handler, fixed indentation and locale keys; values rendered and remembered. |
| TC-06: No-Param Mode | [x] | [x] | [x] | All Platforms Passed | All platforms: Clean transformation without parameter variables, empty bindings rendered in output. |
| TC-07: HTML/TXT/XML Formats | [x] | [x] | [x] | All Platforms Passed | All platforms: HTML, TXT, and XML output method detection verified; appropriate extensions generated and opened. |
| TC-08: Directory Navigation | [x] | [x] | [x] | All Platforms Passed | All platforms: subfolder/ shown and navigable; .hidden/ correctly excluded from quick panel; [Parent Directory] returns correctly to parent folder. |
| TC-09: Cancellation Flow | [x] | [x] | [x] | All Platforms Passed | All platforms: Escape at XSL panel, param mode panel, manual input, and file prompts cleanly aborts with status bar message; no ghost files or tracebacks. |
| TC-10: Error Panel Display | [x] | [x] | [x] | All Platforms Passed | All platforms: Pre-validation catches malformed XML immediately on Ctrl+B/Cmd+B; malformed XSL displays compilation errors in xml_transformer_errors panel and jumps to exact line/column; existing output files preserved. |
| TC-11: Localization Parity | [x] | [x] | [x] | All Platforms Passed | All platforms: 100% key and placeholder parity across en and es locale files; automatic language resolution from OS locale with fallback to English. |
...
- [x] No temporary files (e.g. `test-output.*`, `custom_params.xml`) committed to git.
- [x] `.gitattributes` ensures `test_xslt/` is excluded from Package Control distribution bundle if desired.
- [x] Setup script permissions (`chmod +x`) preserved in tar/zip distributions.
