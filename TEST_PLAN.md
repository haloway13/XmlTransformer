...
| TC-03: Missing Dependency Alert | [x] | [x] | [x] | All Platforms Passed | Found & fixed 1 bug: java_missing/jars_missing dialogs in run() called get_message() with no args, silently dropping the platform-specific install command and setup script path that plugin_loaded() computed (TC-03 requires the JARs dialog show the setup script path). Wired real args through; both dialogs now show correct, informative text; both scenarios verified clean (no traceback) and fully restored after. Windows verified: renaming Saxon directory triggered pop-up dialog identifying C:\Program Files\Saxon and setup_XmlTransformer_windows.bat; restoration verified. macOS: verified directory rename and missing java cases cleanly. |
...
| TC-05: Manual Param Entry | [x] | [x] | [x] | All Platforms Passed | Found & fixed 3 bugs never caught on any platform: missing enter_param_file/enter_param_value locale keys (KeyError crash), missing on_param_value_entered() method (AttributeError crash after 1st param), pretty_print_xml() misindented closing </params> tag. custom_params.xml now saves correctly and test-output.html renders entered values. Windows verified: manual entry prompted for each param, saved to custom_params.xml with proper indentation, rendered in HTML. macOS: params_test_mac.xml saved with correct indentation; values rendered; filename remembered in last_param_filename |
...
| TC-08: Directory Navigation | [x] | [x] | [x] | All Platforms Passed | subfolder/ shown and navigable; .hidden/ correctly excluded from quick panel; Parent Directory returns correctly. Windows verified: subfolder/ listed and navigated; .hidden/ excluded; [Parent Directory] returns to test_xslt/. macOS: subfolder/ listed and navigable (shows its test.xsl); .hidden/ (containing an .xsl) excluded; [Parent Directory] returns to test_xslt/. |
| TC-09: Cancellation Flow | [x] | [x] | [x] | All Platforms Passed | Found & fixed 1 bug: Escape at the manual param-value/filename input panels gave no status bar feedback (on_cancel was None), unlike every quick-panel cancel. All 4 cancellation points now confirmed working with status bar messages, no ghost files, no tracebacks. Windows verified: Esc at XSL panel, param mode panel, and manual input cleanly aborts with status message. macOS: Esc verified at XSL panel, param-mode panel, first param value, filename prompt and param file browser. |
| TC-10: Error Panel Display | [x] | [x] | [x] | All Platforms Passed | Found & fixed 1 bug: invalid XML was only validated deep inside run_transformation() (after XSL + param prompts), not immediately on Ctrl+B; moved validation earlier. Both invalid-XML and invalid-XSL cases now show clean descriptive alerts with no traceback. Windows verified: invalid XML (broken.xml) shows immediate pop-up alert; malformed XSL (broken_xslt.xsl) displays Saxon compilation errors in xml_transformer_errors panel and automatically jumps to line 8 column 61 of broken_xslt.xsl without touching or opening existing test-output.html. macOS: verified invalid XML alert on Cmd+B; non-well-formed XSL alerts before Java runs; well-formed invalid XSLT (XPST0003) shows xml_transformer_errors panel, opens the XSL at line 6 col 45, no stale output opened. |
