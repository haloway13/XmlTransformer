...
- **Fix (macOS)**: `get_java_bin()` only checked hardcoded Homebrew paths, so JDKs installed as `.pkg`/casks under `/Library/Java/JavaVirtualMachines` (Temurin, Zulu, Oracle) were ignored whenever an older Homebrew `openjdk@11` existed — the plugin kept running Java 11 with Java 17 installed. Now asks `/usr/libexec/java_home` for the newest JDK first and uses it when it is Java 17+ (version read from the JDK's `release` file, since `java_home -v 17+` silently returns the default JDK when nothing matches), falling back to the Homebrew paths. The macOS "Java missing" hint now suggests `brew install --cask temurin@17`.
- **Fix (macOS)**: `setup_XmlTransformer_macos.sh` installed Java via the `openjdk@17` formula, which has no prebuilt bottle on older macOS/Intel and fails to build from source without full Xcode (`metal`); under `set -e` the script then exited before downloading the JARs. It now installs the prebuilt `temurin@17` cask first, falls back to the formula, and continues to the JAR step if Java installation fails. It also accepted any `java` on `PATH` (e.g. Java 11) as satisfying the Java 17+ requirement; it now checks the major version. The script is now committed as executable (was mode 644).
- **Fix**: `run_transformation()` re-checks the JAR directory before invoking Saxon but called `get_message("jars_missing")` without its path/setup-script arguments, raising `IndexError` (no dialog shown) if JARs were removed while Sublime Text was running.
- **Fix**: After launching Java, `XmlTransformer_exec` checked for the output file after 500ms. When output from a previous run existed, it was opened while Saxon was still running and error handling was skipped — a failed transformation looked like a success showing stale output, and the `xml_transformer_errors` panel never appeared. Also failed to recognize completion on non-zero exits (Sublime appends `[cmd:]`/`[dir:]`/`[path:]` after `[Finished ...]`) and did not match Saxon 12's error position format. Now waits for the build to finish, decides on the exit code, opens output only on success, and on failure shows the error panel and jumps to the error position in the XSL.
- **Fix (Windows)**: On Windows, Sublime Text's build system sometimes reports `[Finished in ...ms]` without an explicit exit code on process termination. Updated error detection to inspect output content for Saxon compiler errors and exceptions. Fixed an `AttributeError` on `self.xsl_path` in `XmlTransformer_exec.py`, ensured existing output files on disk are never prematurely deleted prior to build completion, and wrapped debug logging behind the `debug` setting.
- **Fix (Windows)**: In `setup_XmlTransformer_windows.bat`, replaced `%ERRORLEVEL%` with delayed expansion `!ERRORLEVEL!` inside parenthesized blocks so curl failure detection correctly validates downloads.
